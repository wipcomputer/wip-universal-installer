...
But the users are changing. AI agents are the new users. They don't click. They call functions. They read instructions. They compose tools.
...
So every tool should expose multiple "doors" ... interfaces into the same core logic:
...
Not every tool needs all six. Build what makes sense. But the more doors you open, the more agents can use your tool.
...
`wip-install` scans any repo, detects which doors exist, and installs them all. One command.
...
```
  Installing: wip-grok
  ────────────────────────────────────────
  Detected 4 door(s): cli, module, mcp, skill
  ...
  Done. 4 door(s) processed.
```

### What It Detects

| Pattern | Door | Install action |
|---------|------|---------------|
